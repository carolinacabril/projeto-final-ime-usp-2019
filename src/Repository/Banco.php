...
class Banco {
	...
	public function getRandom2Products() {
		$strsql = "SELECT * FROM produtos ORDER BY RAND() LIMIT 0, 2";
	...
	public function pesquisa($texto) {
		$strsql = "select * from produtos where nome like '%" . $texto . "%' or descricao like '%" . $texto . "%'";
	...
	public function criarLogin($nome, $email, $senha) {
		$strsql = "insert into clientes (nome, email, telefone, senha) values ('" . $nome . "', '" . $email . "', '', '" . md5($senha) . "')";
		$resultados = $this->getResultsBD($strsql);
		return $resultados;
	}
