...
class LojaController extends AbstractController {
	...
	*/
	public function index() {
		$banco = new Banco();
		$produtos = $banco->getRandom2Products();
	...
	*/
	public function pesquisa(Request $request) {
		...
		$banco = new Banco();
		$produtos = $banco->pesquisa($texto);

		return $this->render('loja/pesquisa.html.twig', [
			'texto' => $texto,
			'produtos' => $produtos
		]);
	}
	...
	/**
	* @Route("/loja/finalizar", name="loja_finalizar")
	*/
	...
	*/
	public function carrinhoAlterar($id, Request $request, SessionInterface $session) {
		...
		$carrinho = $session->get('carrinho');
		$quantidadeParam = $request->request->get('quantidade');
		if (is_array($carrinho) && array_key_exists($id, $carrinho)) {
			if($quantidadeParam == 0) {
				unset($carrinho[$produto->getId()]);
			} else {
				$totalItem = $quantidadeParam * $produto->getPreco();
				$carrinho[$produto->getId()]['quantidade'] = $quantidadeParam;
